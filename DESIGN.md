...
- **Sequence:** Finds form one centered reading column ordered by the time Justin saved them, not by the original publication date.
...
- **Source layer:** Preserve the identity and useful metadata of each source. X posts lead with the author's real profile image, name, handle, publication date, post text, and canonical URL; future articles and podcasts follow the same evidence-first pattern.
- **Outbound action:** Use one quiet external-arrow control with a complete accessible label; do not repeat the destination beside an already-understood source item.
...
- **Failure:** If an original source becomes unavailable, keep Justin's thought and replace its snapshot with a concise unavailable state.
